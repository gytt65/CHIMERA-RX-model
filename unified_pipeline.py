...
class UnifiedPricingPipeline:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        # Full→fast distillation state (live path uses this lightweight correction).
        self._fast_distill_ratio = 1.0
        self._fast_distill_updates = 0
    ...
    def price(
        ...
        mode : str — "fast" (low-latency live) or "full" (training/recalibration)
        ...
        if mode_norm not in ("fast", "full"):
            mode_norm = "fast"
        fast_mode = bool(mode_norm == "fast")

        diagnostics: Dict[str, Any] = {'stages_run': [], 'stages_skipped': []}
        diagnostics['mode'] = mode_norm
        diagnostics['distilled_from_full'] = bool(fast_mode)
        ...
        if fast_mode:
            diagnostics['stages_skipped'].append('sgm: fast_mode')
        elif 'sgm' in self._components and self._components['sgm'].is_fitted:
            try:
            ...
            try:
                if fast_mode:
                    if not pinn.is_fitted:
                        diagnostics['stages_skipped'].append('pinn: fast_mode_not_fitted')
                    else:
                        diagnostics['stages_skipped'].append('pinn_refit: fast_mode')
                else:
        ...
        if fast_mode:
            if self._hawkes_last:
                hawkes_params = self._hawkes_last
                hawkes_cluster = float(hawkes_params.get('clustering_score', 0.0))
                diagnostics['stages_skipped'].append('hawkes: fast_mode_reuse_last')
            else:
                diagnostics['stages_skipped'].append('hawkes: fast_mode_no_state')
        elif 'hawkes' in self._components and historical_returns is not None:
            try:
        ...
        if fast_mode:
            diagnostics['stages_skipped'].append('neural_jsde: fast_mode')
        elif 'neural_jsde' in self._components:
            try:
        ...
        if fast_mode:
            diagnostics['stages_skipped'].append('rbergomi: fast_mode')
        elif 'rbergomi' in self._components:
            try:
        ...
        _fast_anchor = float(
            component_prices.get(
                'heston_cos',
                component_prices.get('bsm', max(final_price, 1e-6))
            )
        )
        if not fast_mode:
            ratio = float(final_price / max(_fast_anchor, 1e-6))
            ratio = float(np.clip(ratio, 0.5, 1.8))
            self._fast_distill_ratio = float(0.90 * self._fast_distill_ratio + 0.10 * ratio)
            self._fast_distill_updates += 1
            diagnostics['distilled_from_full'] = False
        else:
            if self._fast_distill_updates > 0:
                final_price = float(max(0.0, final_price * self._fast_distill_ratio))
            diagnostics['distilled_from_full'] = bool(self._fast_distill_updates > 0)
        diagnostics['fast_distill_ratio'] = round(float(self._fast_distill_ratio), 6)
        diagnostics['fast_distill_updates'] = int(self._fast_distill_updates)
            ...
            try:
                ...
                if fast_mode:
                    ...
                    diagnostics['stages_skipped'].append(
                        'deep_hedging: fast_mode_using_bsm_delta'
                    )
                elif hedger.is_trained:
                    hedge_state = {
                        'log_moneyness': float(np.log(strike / spot)),
                        'time_to_expiry': float(T),
        ...
        return {
            ...
            'distilled_from_full': bool(diagnostics.get('distilled_from_full', False)),
    ...
    def pipeline_status(self) -> Dict:
        ...
        status['metadata'] = {
            ...
            'hurst': round(float(self._hurst), 4),
        }
        return status
