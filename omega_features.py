...
The module-level singleton defaults to a conservative research baseline
that keeps core safeguards ON (conformal intervals, OOS gate, drift
guard, high-conviction filtering) while leaving heavier modules OFF.
Individual flags can be overridden via:
...
class OmegaFeatures:
    ...
    _ALIASES = {
        ...
        'use_ml_ensemble': 'USE_ML_ENSEMBLE',
        ...
        'use_chimera_meta_overlay': 'USE_CHIMERA_META_OVERLAY',
    }
...
# Module-level singleton defaults to a practical research baseline.
# UI/runtime can still override via `set_features(...)` or profile helpers.
