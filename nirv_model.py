...
try:
    from calibration.regime_conformal import RegimeConformalIntervals
except Exception:
    RegimeConformalIntervals = None

try:
    from meta.residual_stacker import ResidualStacker
except Exception:
    ResidualStacker = None
...
NirvOutput = namedtuple('NirvOutput', [
    ...
    'fair_value_fast',      # Fast-path fair value (INR)
    'fair_value_full',      # Full-path fair value (INR)
    ...
    'interval_low',         # Lower calibrated interval bound
    'interval_high',        # Upper calibrated interval bound
    'expected_net_edge_pct',# Edge after transaction costs in %
    'route_used',           # Tier/pricer route used
    'latency_ms',           # End-to-end latency for this quote
])
...
class HestonJumpDiffusionPricer:
    ...
    def price(self, spot, strike, T, r, q, sigma, regime_params,
              option_type='CE', india_features=None, garch_vol=None):
        """
        ...
        # Blend regime theta_v with market and optional GARCH variance so
        # long-run variance adapts to both implied and realized-vol forecasts.
        if garch_vol is not None and np.isfinite(garch_vol) and garch_vol > 0:
            theta_v = (
                0.35 * theta_v_regime
                + 0.35 * sigma**2
                + 0.30 * float(garch_vol)**2
            )
        else:
        ...
        if garch_vol is not None and np.isfinite(garch_vol) and garch_vol > 0:
            _garch_var = float(garch_vol) ** 2
            _regime_theta = np.array([
                0.35 * RegimeDetector.REGIME_PARAMS[r].get('theta_v', sigma**2)
                + 0.35 * sigma**2
                + 0.30 * _garch_var
            ...
            _regime_theta = np.array([
                0.5 * RegimeDetector.REGIME_PARAMS[r].get('theta_v', sigma**2) + 0.5 * sigma**2
                for r in _regime_names
            ])
        ...
        cum_probs = np.cumsum(_trans_step, axis=1) if _use_regime_switching else None
        ...
        for step in range(n_steps):
            ...
            if _use_regime_switching and step > 0:
                ...
                thresholds = cum_probs[regime_idx]  # (n_paths, n_regimes)
                ge_mask = thresholds >= u_regime[:, None]
                regime_idx = np.argmax(ge_mask, axis=1)
                no_hit = ~np.any(ge_mask, axis=1)
                if np.any(no_hit):
                    regime_idx[no_hit] = _n_regimes - 1
                regime_idx = np.clip(regime_idx, 0, _n_regimes - 1).astype(int, copy=False)
        ...
        # Guard extreme/invalid paths so payoff moments stay finite.
        if not np.all(np.isfinite(S_T)):
            finite_mask = np.isfinite(S_T)
            if np.any(finite_mask):
                cap = float(np.percentile(S_T[finite_mask], 99.9))
            else:
                cap = float(max(spot * 10.0, 1.0))
            S_T = np.where(finite_mask, S_T, cap)
        if not np.all(np.isfinite(S_T_gbm)):
            finite_mask = np.isfinite(S_T_gbm)
            if np.any(finite_mask):
                cap = float(np.percentile(S_T_gbm[finite_mask], 99.9))
            else:
                cap = float(max(spot * 10.0, 1.0))
            S_T_gbm = np.where(finite_mask, S_T_gbm, cap)
        ...
        self._last_feller_ratio = float(feller_ratio)
        self._last_S_T = S_T
...
class BayesianConfidenceEngine:
    ...
    def compute_profit_probability(self, S_terminal, strike, market_price,
        ...
        q            : float - Dividend/carry yield (optional, default 1.2%)
        ...
        if spot is not None and spot > 0:
            ...
            q_est = float(q) if q is not None and np.isfinite(q) else 0.012
            ...
            S_safe = np.clip(np.asarray(S_terminal, dtype=float), 1e-12, None)
            if vrp_var > 0 and iv_squared > 1e-8 and spot > 0:
                # Conservative variance-scaling around spot in log-space.
                variance_ratio = float(np.clip(rv_squared / max(iv_squared, 1e-12), 0.30, 1.00))
                sqrt_ratio = np.sqrt(variance_ratio)
                log_rel = np.log(S_safe / max(float(spot), 1e-8))
                log_S_physical = np.log(max(float(spot), 1e-8)) + sqrt_ratio * log_rel
                S_T_physical = np.exp(log_S_physical + drift_shift)
            else:
                variance_shift = -0.5 * vrp_var * T  # fallback mean-shift
                S_T_physical = S_safe * np.exp(drift_shift + variance_shift)
            if not np.all(np.isfinite(S_T_physical)):
                finite_mask = np.isfinite(S_T_physical)
                if np.any(finite_mask):
                    cap = float(np.percentile(S_T_physical[finite_mask], 99.9))
                else:
                    cap = float(max(spot * 10.0, 1.0))
                S_T_physical = np.nan_to_num(
                    S_T_physical,
                    nan=max(float(spot), 1e-8),
                    posinf=cap,
                    neginf=1e-12,
                )
            ...
            if iv is not None and returns_30d is not None and len(returns_30d) >= 10:
                ...
                if vrp > 0.005:  # Significant positive VRP
                    ...
                    vrp_adj = min(vrp * vrp_scale, 5.0)  # cap at 5pp
                    if is_call:
                    ...
                    vrp_adj = min(abs(vrp) * vrp_scale, 5.0)
                    physical_profit_prob += vrp_adj * 0.5
        ...
        # Use a smooth sigmoid mapping to avoid brittle linear scaling.
        ci_width = (ci_high - ci_low) / max(market_price, 0.01)
        logits = np.clip(8.0 * ci_width - 3.5, -60.0, 60.0)
        confidence = float(np.clip(100.0 / (1.0 + np.exp(logits)), 0.0, 99.0))
...
class NIRVModel:
    ...
    def __init__(self, n_paths=50000, n_bootstrap=5000, returns_history=None, vix_history=None):
        ...
        self._hmm_trained_once = False
        if returns_history is not None:
            try:
                hist = np.asarray(returns_history, dtype=float)
                hist = hist[np.isfinite(hist)]
                if len(hist) >= 250:
                    self._hmm_trained_once = bool(
                        self.regime_detector.train_hmm(hist, vix_history=vix_history)
                    )
            except Exception:
                self._hmm_trained_once = False
        ...
        if QUANT_ENGINE_AVAILABLE:
            try:
                ...
                pass
        self._residual_stacker = ResidualStacker() if ResidualStacker is not None else None
        self._residual_history: List = []
        self._conformal = RegimeConformalIntervals(alpha=0.10, min_bucket_samples=20) if RegimeConformalIntervals is not None else None
        self._conformal_history: List = []
    ...
    def price_option(self, spot, strike, T, r, q, option_type, market_price,
        ...
        futures_price = kwargs.get("futures_price", None)
        q_confidence = q
        try:
            if futures_price is not None:
                fwd = float(futures_price)
                if np.isfinite(fwd) and fwd > 0 and spot > 0 and T > 1e-8:
                    implied_q = r - np.log(fwd / spot) / T
                    if np.isfinite(implied_q):
                        # Keep carry realistic for index options while allowing mild backwardation.
                        q_confidence = float(np.clip(implied_q, -0.05, 0.08))
        except Exception:
            q_confidence = q

        # Opportunistic HMM training when a longer history is provided.
        if not self._hmm_trained_once:
            hist_returns = kwargs.get("returns_history", kwargs.get("returns_252d"))
            if hist_returns is None and returns_30d is not None and len(returns_30d) >= 250:
                hist_returns = returns_30d
            hist_vix = kwargs.get("vix_history")
            if hist_returns is not None:
                try:
                    hist_arr = np.asarray(hist_returns, dtype=float)
                    hist_arr = hist_arr[np.isfinite(hist_arr)]
                    if len(hist_arr) >= 250:
                        self._hmm_trained_once = bool(
                            self.regime_detector.train_hmm(hist_arr, vix_history=hist_vix)
                        )
                except Exception:
                    pass
        ...
        regime_params = dict(RegimeDetector.REGIME_PARAMS[regime])
        regime_params["_regime_name"] = regime
        regime_blend = {
            "enabled": False,
            "primary": regime,
            "secondary": None,
            "primary_prob": float(regime_probs.get(regime, 0.0)),
        }
        sorted_regimes = sorted(
            [(k, float(v)) for k, v in (regime_probs or {}).items()],
            key=lambda x: x[1],
            reverse=True,
        )
        if len(sorted_regimes) > 1 and sorted_regimes[0][1] < 0.70:
            r1, p1 = sorted_regimes[0]
            r2, p2 = sorted_regimes[1]
            total_p = max(p1 + p2, 1e-12)
            blended = {}
            for key in RegimeDetector.REGIME_PARAMS[r1]:
                v1 = RegimeDetector.REGIME_PARAMS[r1].get(key)
                v2 = RegimeDetector.REGIME_PARAMS[r2].get(key)
                if isinstance(v1, (int, float)) and isinstance(v2, (int, float)):
                    blended[key] = (p1 * float(v1) + p2 * float(v2)) / total_p
                else:
                    blended[key] = v1
            blended["_regime_name"] = r1  # dominant state for MC switching start
            regime_params = blended
            regime = r1
            regime_blend = {
                "enabled": True,
                "primary": r1,
                "secondary": r2,
                "primary_prob": float(p1),
                "secondary_prob": float(p2),
            }
        ...
        quant_extras = {}
        quant_extras['regime_blend'] = regime_blend
        if isinstance(vrp_state, dict):
        ...
        fair_value_fast = np.nan
        fair_value_full = np.nan
        route_used = "mc_full"
        if getattr(get_features(), "USE_TIERED_PRICER", False) and self.pricer_router is not None:
            ...
            route_used = str(route.get("tier_used", "tier0_surface_bs"))
            fair_value_fast = float(
                route.get("cos_price")
                if np.isfinite(route.get("cos_price", np.nan)) and route.get("cos_price", np.nan) > 0
                else route.get("bs_price", fair_value)
            )
            fair_value_full = float(fair_value)
            ...
            if S_terminal is None or len(np.asarray(S_terminal).reshape(-1)) < 10:
                ...
                if hasattr(self.pricer, "_rng") and self.pricer._rng is not None:
                    z = self.pricer._rng.standard_normal(n_proxy)
                else:
            ...
            fair_value, std_error, S_terminal = self.pricer.price(
                ...
                garch_vol=quant_extras.get('garch_vol')
            )
            fair_value_fast = float(quant_extras.get('heston_cos_price', fair_value))
            fair_value_full = float(fair_value)
            route_used = "mc_full"
        ...
        (profit_prob_rn, physical_profit_prob, confidence,
         expected_pnl_rn, physical_expected_pnl, fv_dist) = \
            self.confidence_engine.compute_profit_probability(
                ...
                regime=regime, iv=sigma, q=q_confidence
            )
        ...
        # 6a2. Residual post-corrector and split-conformal interval head.
        residual_features = {
            "log_moneyness": float(np.log(max(strike, 1e-6) / max(spot, 1e-6))),
            "dte": float(days_to_expiry),
            "iv": float(sigma),
            "vix": float(india_vix),
            "spread_ratio": float(
                max(float(kwargs.get("ask", market_price)) - float(kwargs.get("bid", market_price)), 0.0)
                / max(market_price, 1e-6)
            ),
            "volume_oi_ratio": float(kwargs.get("volume_oi_ratio", 0.1)),
            "regime_bull_low": 1.0 if regime == "Bull-Low Vol" else 0.0,
            "regime_bear_high": 1.0 if regime == "Bear-High Vol" else 0.0,
            "regime_sideways": 1.0 if regime == "Sideways" else 0.0,
            "regime_bull_high": 1.0 if regime == "Bull-High Vol" else 0.0,
        }
        if self._residual_stacker is not None and self._residual_stacker.is_fitted:
            try:
                fair_value += float(self._residual_stacker.predict(residual_features))
                fair_value_full = float(fair_value)
                quant_extras["residual_stacker_applied"] = True
            except Exception:
                quant_extras["residual_stacker_applied"] = False

        interval_low = max(0.01, float(fair_value - 1.64 * max(std_error, 0.01)))
        interval_high = max(interval_low, float(fair_value + 1.64 * max(std_error, 0.01)))
        if self._conformal is not None:
            try:
                interval_low, interval_high, q_abs = self._conformal.interval(
                    y_pred=float(fair_value),
                    regime=str(regime),
                    log_moneyness=float(np.log(max(strike, 1e-6) / max(spot, 1e-6))),
                    dte=int(days_to_expiry),
                    scale=float(kwargs.get("conformal_scale", 1.0)),
                )
                quant_extras["conformal_abs_q"] = float(q_abs)
            except Exception:
                pass
        ...
        signal, mispricing_pct, tc_details = self.signal_gen.generate_signal(
            ...
            bid=kwargs.get("bid"), ask=kwargs.get("ask"),
            fees_per_lot=float(kwargs.get("fees_per_lot", 35.0)),
            lot_size=int(self.lot_size),
        )
        edge_per_unit = float(abs(fair_value - market_price))
        tc_per_unit = float((tc_details or {}).get("tc_per_unit", 0.0))
        expected_net_edge_pct = float(100.0 * (edge_per_unit - tc_per_unit) / max(market_price, 1e-6))
        ...
        greeks['quant_extras']['feller_violated'] = bool(getattr(self.pricer, '_last_feller_violated', False))
        greeks['quant_extras']['feller_ratio'] = float(getattr(self.pricer, '_last_feller_ratio', np.nan))
        ...
        self.state["pricer_route"] = route_used
        if not np.isfinite(fair_value_fast):
            fair_value_fast = float(fair_value)
        if not np.isfinite(fair_value_full):
            fair_value_full = float(fair_value)
        latency_ms = (time.perf_counter() - _tic) * 1000.0

        return NirvOutput(
            fair_value=round(fair_value, 2),
            fair_value_fast=round(float(fair_value_fast), 2),
            fair_value_full=round(float(fair_value_full), 2),
            ...
            interval_low=round(float(interval_low), 2),
            interval_high=round(float(interval_high), 2),
            expected_net_edge_pct=round(float(expected_net_edge_pct), 2),
            route_used=str(route_used),
