...
class HestonCOS:
    ...
    def _cumulant_c4_numerical(self, S, K, T, r, q, V0, kappa, theta, sigma_v, rho):
        ...
        try:
            ...
            # 5-point central difference: the +6*f(0) term is zero because
            # f(0) = ln(phi(0)) = ln(1) = 0 by definition.
...
class QuantEngine:
    ...
    def __init__(self, deterministic_mode=False, seed=42):
        ...
        # Session cache for shared calibrated bundles.
        self._bundle_cache: Dict[Tuple, Dict] = {}
        self._bundle_cache_ttl_s: float = 2.0
    ...
    def _bundle_cache_key(
        ...
        return (
            ...
            round(float(market_price), 6) if market_price is not None else None,
            round(float(vix), 6) if vix is not None else None,
            bool(self.deterministic_mode),
        )
    ...
    def predict_bundle(
        ...
        vix=None,
        returns=None,
        use_adaptive_pde=False,
        cache_ttl_s=None,
        **kwargs,
    ):
        """
        Shared baseline bundle for NIRV/OMEGA/NOVA/CHIMERA callers.

        Returns consistent calibrated artifacts:
          - core_price
          - regime bundle
          - jump/garch/heston diagnostics
          - confidence / ensemble features
        """
        import time as _time

        ttl = float(self._bundle_cache_ttl_s if cache_ttl_s is None else max(cache_ttl_s, 0.0))
        key = self._bundle_cache_key(S, K, T, r, q, sigma, option_type, market_price, vix)
        now = _time.time()
        cached = self._bundle_cache.get(key)
        if cached is not None and (now - float(cached.get("_ts", 0.0))) <= ttl:
            out = dict(cached)
            out["cache_hit"] = True
            return out

        result = self.enhanced_price(
            S=S,
            K=K,
            T=T,
            r=r,
            ...
            vix=vix,
            returns=returns,
            use_adaptive_pde=bool(use_adaptive_pde),
            **kwargs,
        )
        core_price = result.get("ensemble_price")
        if core_price is None:
            core_price = result.get("heston_cos_price")
        if core_price is None:
            core_price = result.get("sabr_vol")
        if core_price is None:
            core_price = market_price

        bundle = {
            "core_price": float(core_price) if core_price is not None else None,
            "blended_vol": float(result.get("blended_vol", sigma)),
            "regime": result.get("regime", {}),
            "hmm_regime": result.get("hmm_regime"),
            "garch_vol": result.get("garch_vol"),
            "garch_vs_iv": result.get("garch_vs_iv"),
            "jump_params": result.get("jump_params"),
            "heston_cos_price": result.get("heston_cos_price"),
            "ensemble_price": result.get("ensemble_price"),
            ...
            "adaptive_pde": result.get("adaptive_pde"),
            "status": self.get_status(),
            "cache_hit": False,
            "_ts": now,
        }
        self._bundle_cache[key] = dict(bundle)
        return bundle
