...
class FeatureFactory:
    ...
    def extract(market_data: dict) -> dict:
        ...
        if isinstance(_ts, datetime.datetime):
            ...
            try:
                _dow = datetime.datetime.fromtimestamp(float(_ts)).weekday()
            except Exception:
                _dow = None
            ...
            if _s:
                try:
                    _dow = datetime.datetime.fromisoformat(_s.replace("Z", "+00:00")).weekday()
                except Exception:
                    try:
                        import pandas as _pd
                        _dow = int(_pd.to_datetime(_s, utc=False).weekday())
                    except Exception:
                        _dow = None
        if _dow is None:
...
class MLPricingCorrector:
    ...
    Cold-start:  returns (0, 0) until ≥ MIN_SAMPLES_BAYESIAN points exist.
    """

    MIN_SAMPLES_BAYESIAN = 10
    MIN_SAMPLES_GBM = 30
    # Backward-compatible alias used in status views and external callers.
    MIN_SAMPLES = MIN_SAMPLES_GBM
    ...
    def __init__(self, model_path='omega_data/pricing_model.joblib'):
        ...
        self._model_stage = 'none'  # one of: none|bayesian|gbm
        ...
        # Optional ensemble state (flag-gated; keeps base model behavior intact).
        self._ensemble_models = {}
        self._ensemble_errors_global = {}
        self._ensemble_errors_regime = {}
    ...
    def _load_model(self):
        ...
        try:
            ...
            self._model_stage = str(saved.get('model_stage', '') or '')
            if self._model_stage not in {'bayesian', 'gbm'}:
                self._model_stage = 'gbm' if len(self.training_X) >= self.MIN_SAMPLES_GBM else 'bayesian'
            self._conformal_q_global = saved.get('conformal_q_global', None)
            self._conformal_q_by_regime = saved.get('conformal_q_by_regime', {})
            self._ensemble_models = saved.get('ensemble_models', {}) or {}
            self._ensemble_errors_global = {
                str(k): deque([float(vv) for vv in (vals or [])], maxlen=300)
                for k, vals in (saved.get('ensemble_errors_global', {}) or {}).items()
            }
            _reg_err = {}
            for model_name, reg_map in (saved.get('ensemble_errors_regime', {}) or {}).items():
                _reg_err[str(model_name)] = {
                    str(reg): deque([float(vv) for vv in (vals or [])], maxlen=300)
                    for reg, vals in (reg_map or {}).items()
                }
            self._ensemble_errors_regime = _reg_err
    ...
    def _save_model(self):
        ...
        try:
            ...
            joblib.dump({
                ...
                'ensemble_errors_global': {
                    str(k): list(v)[-300:]
                    for k, v in (self._ensemble_errors_global or {}).items()
                },
                'ensemble_errors_regime': {
                    str(m): {str(r): list(vals)[-300:] for r, vals in (reg_map or {}).items()}
                    for m, reg_map in (self._ensemble_errors_regime or {}).items()
                },
    ...
    def predict_correction(self, features: dict):
        ...
        try:
            ...
            # Optional hybrid ensemble for >=30 samples (flag-gated).
            if (
                bool(getattr(get_features(), "USE_ML_ENSEMBLE", False))
                and isinstance(self._ensemble_models, dict)
                and len(self._ensemble_models) >= 2
                and self._model_stage == 'gbm'
            ):
                regime = 'Sideways'
                if float(features.get('regime_bull_low', 0.0)) > 0.5:
                    regime = 'Bull-Low Vol'
                elif float(features.get('regime_bear_high', 0.0)) > 0.5:
                    regime = 'Bear-High Vol'
                elif float(features.get('regime_bull_high', 0.0)) > 0.5:
                ...
                for name, model in self._ensemble_models.items():
                    try:
                        ...
                        continue
                    preds[str(name)] = pred
                    reg_hist = (((self._ensemble_errors_regime or {}).get(str(name), {}) or {}).get(regime))
                    glob_hist = ((self._ensemble_errors_global or {}).get(str(name)))
                    if reg_hist is not None and len(reg_hist) >= 8:
                        mse = float(np.mean(np.asarray(list(reg_hist), dtype=float)))
                    elif glob_hist is not None and len(glob_hist) >= 8:
                        mse = float(np.mean(np.asarray(list(glob_hist), dtype=float)))
                    else:
                        mse = 0.01
                    weights[str(name)] = 1.0 / max(mse, 1e-6)

                if preds:
                    wsum = max(float(sum(weights.values())), 1e-8)
                    correction = float(sum(preds[n] * weights[n] for n in preds) / wsum)
                    disagreement = float(np.std(np.asarray(list(preds.values()), dtype=float)))

            # Warm-start model is intentionally conservative.
            if self._model_stage == 'bayesian':
            ...
            confidence = float(getattr(self, '_cv_confidence', min(0.85, len(self.training_X) / 500.0)))
            if disagreement > 0.0:
                # Penalize confidence when ensemble models disagree.
                confidence *= float(np.clip(1.0 / (1.0 + 8.0 * disagreement), 0.55, 1.0))
    ...
    def predict_correction_with_interval(self, features: dict, alpha: float = 0.10):
        ...
        raw_mult = moneyness_mult * time_mult * regime_mult * gamma_flip_mult
        total_mult = float(np.clip(raw_mult, 1.0, 3.35))
        adaptive_q = q * total_mult
    ...
    def add_sample(self, features: dict, residual: float, weight: float = 1.0):
        ...
        if not hasattr(self, 'training_w'):
            self.training_w = [1.0] * len(self.training_y)
            self.training_w[-1] = float(np.clip(weight, 0.0, 1.0))
        else:
        ...
        if n >= self.MIN_SAMPLES_BAYESIAN and n % retrain_interval == 0:
            self._train()

    @staticmethod
    def _regime_name_from_row(x_row: np.ndarray, names: list) -> str:
        try:
            idx_bl = names.index('regime_bull_low')
            idx_bh = names.index('regime_bear_high')
            idx_sw = names.index('regime_sideways')
            idx_bhv = names.index('regime_bull_high')
        except ValueError:
            return 'Sideways'
        vals = {
            'Bull-Low Vol': float(x_row[idx_bl]),
            'Bear-High Vol': float(x_row[idx_bh]),
            'Sideways': float(x_row[idx_sw]),
            'Bull-High Vol': float(x_row[idx_bhv]),
        }
        return max(vals, key=vals.get)

    def _effective_training_weights(self, X_raw: np.ndarray, base_w: np.ndarray) -> np.ndarray:
        """
        Blend explicit sample weights with recency + liquidity weighting.

        This improves adaptivity without discarding existing training history.
        """
        n = len(base_w)
        if n == 0:
            return base_w
        # Half-life at ~20% of the sample history.
        hl = max(float(n) * 0.20, 20.0)
        age = np.arange(n, dtype=float)
        recency = np.exp(-(n - 1 - age) * np.log(2.0) / hl)

        names = FeatureFactory.get_feature_names()
        liq_mult = np.ones(n, dtype=float)
        if X_raw.ndim == 2 and X_raw.shape[0] == n and 'volume_oi_ratio' in names:
            liq_idx = names.index('volume_oi_ratio')
            liq = np.clip(X_raw[:, liq_idx], 0.0, 1.0)
            liq_mult = 0.80 + 0.40 * liq  # [0.8, 1.2]

        w_eff = np.asarray(base_w, dtype=float) * recency * liq_mult
        w_eff = np.clip(w_eff, 1e-4, None)
        w_eff *= float(n) / max(float(np.sum(w_eff)), 1e-8)  # normalize mean weight to 1
        return w_eff

    def _refresh_ensemble_models(self, X_s: np.ndarray, y: np.ndarray, w: np.ndarray):
        """
        Train lightweight ensemble models and refresh recent error stats.
        """
        self._ensemble_models = {}
        self._ensemble_errors_global = {}
        self._ensemble_errors_regime = {}

        if not bool(getattr(get_features(), "USE_ML_ENSEMBLE", False)):
            return
        if len(y) < self.MIN_SAMPLES_GBM:
            return
        if BayesianRidge is None:
            return

        models = {}
        # Reuse primary model as GBM/LGBM leg where possible.
        if self.model is not None:
            models['gbm'] = self.model

        bayes = BayesianRidge(alpha_1=1e-3, lambda_1=1e-3)
        try:
            bayes.fit(X_s, y, sample_weight=w)
        except TypeError:
            bayes.fit(X_s, y)
        models['bayesian'] = bayes
        self._ensemble_models = models

        names = FeatureFactory.get_feature_names()
        X_raw = np.asarray(self.training_X, dtype=float)
        start = max(0, len(y) - 400)  # recent-window diagnostics
        y_recent = y[start:]
        x_recent_scaled = X_s[start:]
        x_recent_raw = X_raw[start:] if X_raw.ndim == 2 and X_raw.shape[0] >= len(y) else None

        for model_name, model in models.items():
            try:
                pred_recent = np.asarray(model.predict(x_recent_scaled), dtype=float)
            except Exception:
                continue
            sq_err = np.square(y_recent - pred_recent)
            self._ensemble_errors_global[model_name] = deque(sq_err.tolist(), maxlen=300)

            reg_map = defaultdict(lambda: deque(maxlen=300))
            if x_recent_raw is not None and len(x_recent_raw) == len(sq_err):
                for idx, err in enumerate(sq_err):
                    reg = self._regime_name_from_row(x_recent_raw[idx], names)
                    reg_map[reg].append(float(err))
            self._ensemble_errors_regime[model_name] = dict(reg_map)

    def _train(self):
        n_samples = len(self.training_X)
        if not SKLEARN_AVAILABLE or n_samples < self.MIN_SAMPLES_BAYESIAN:
            return
        if self.model is not None:
            try:
        ...
        base_w = np.array(
            self.training_w
            if hasattr(self, 'training_w') and len(getattr(self, 'training_w', [])) == len(y)
            else [1.0] * len(y)
        )
        w = self._effective_training_weights(X, base_w)
        self.scaler = StandardScaler()
        X_s = self.scaler.fit_transform(X)
        oof_preds = np.full_like(y, np.nan)

        # Phase A (10-29 samples): Bayesian ridge warm-start.
        if n_samples < self.MIN_SAMPLES_GBM:
            if BayesianRidge is None:
                return
            model = BayesianRidge(alpha_1=1e-3, lambda_1=1e-3)
            try:
                model.fit(X_s, y, sample_weight=w)
            except TypeError:
                model.fit(X_s, y)
            self.model = model
            self._model_stage = 'bayesian'
            preds = model.predict(X_s)
            oof_preds[:] = preds
            score = float(model.score(X_s, y))
            self._cv_confidence = float(np.clip(max(score, 0.0), 0.0, 0.75))
        else:
            # Phase B (>=30 samples): tree model with LightGBM->GBM fallback.
            ...
            def _fit_with_fallback(model, x_fit, y_fit, w_fit):
                try:
                    model.fit(x_fit, y_fit, sample_weight=w_fit)
                    return model
                except Exception:
                    fb = GradientBoostingRegressor(
                        n_estimators=min(150, max(30, len(y_fit) // 3)),
                        max_depth=4,
                        learning_rate=0.08,
                        subsample=0.8,
                        random_state=42,
                    )
                    fb.fit(x_fit, y_fit, sample_weight=w_fit)
                    return fb

            for train_idx, val_idx in tscv.split(X_s):
                fold_model = _make_model(len(train_idx))
                fold_model = _fit_with_fallback(
                    fold_model, X_s[train_idx], y[train_idx], w[train_idx]
                )
            ...
            final_model = _make_model(len(y))
            self.model = _fit_with_fallback(final_model, X_s, y, w)
            self._model_stage = 'gbm'
            self._refresh_ensemble_models(X_s, y, w)
    ...
    def get_feature_importance(self):
        ...
        if hasattr(self.model, 'coef_'):
            coef = np.abs(np.ravel(self.model.coef_))
            if coef.size == len(names):
                return {
                    name: round(float(v), 4)
                    for name, v in zip(names, coef)
                    if v > 0.005
                }
        return {}
...
class PredictionTracker:
    ...
    # ------------------------------------------------------------------
    def record(self, pred_id, features, prediction, meta=None):
        meta = meta or {}
        ts_epoch = meta.get("ts_epoch", None)
        now_dt = datetime.datetime.now()
        try:
            if ts_epoch is not None:
                t = float(ts_epoch)
                if np.isfinite(t):
                    now_dt = datetime.datetime.fromtimestamp(t)
        except Exception:
            pass
    ...
    @staticmethod
    def _ewma_metrics_for_records(records, half_life: float = 60.0):
        """
        Recency-weighted accuracy and edge to avoid stale-history dominance.
        """
        if not records:
            return 0.0, 0.0
        n = len(records)
        age = np.arange(n, dtype=float)
        w = np.exp(-(n - 1 - age) * np.log(2.0) / max(float(half_life), 1.0))
        w = np.clip(w, 1e-8, None)
        correct = np.array([
            1.0 if (
                ('BUY' in str(r.get('signal', '')).upper() and float(r.get('signed_return_pct', 0.0)) > 0.0)
                or ('SELL' in str(r.get('signal', '')).upper() and float(r.get('signed_return_pct', 0.0)) > 0.0)
            ) else 0.0
            for r in records
        ], dtype=float)
        signed_ret = np.array([float(r.get('signed_return_pct', 0.0)) for r in records], dtype=float)
        wsum = float(np.sum(w))
        acc = 100.0 * float(np.sum(w * correct)) / max(wsum, 1e-8)
        edge = float(np.sum(w * signed_ret)) / max(wsum, 1e-8)
        return acc, edge
    ...
    def get_reliability_gate_decision(
        ...
        accuracy_raw = self._accuracy_for_records(rows)
        avg_edge_raw = float(np.mean([r['signed_return_pct'] for r in rows])) if rows else 0.0
        accuracy_ewma, avg_edge_ewma = self._ewma_metrics_for_records(rows)
        accuracy_pct = 0.35 * accuracy_raw + 0.65 * accuracy_ewma
        avg_edge_pct = 0.35 * avg_edge_raw + 0.65 * avg_edge_ewma

        side = 'BUY' if 'BUY' in sig else 'SELL'
        side_rows = [r for r in rows if side in r['signal']]
        side_samples = len(side_rows)
        side_acc_raw = self._accuracy_for_records(side_rows) if side_rows else 0.0
        side_acc_ewma, _ = self._ewma_metrics_for_records(side_rows) if side_rows else (0.0, 0.0)
        side_accuracy_pct = 0.35 * side_acc_raw + 0.65 * side_acc_ewma
        ...
        reg_acc_raw = self._accuracy_for_records(regime_rows) if regime_rows else 0.0
        reg_acc_ewma, _ = self._ewma_metrics_for_records(regime_rows) if regime_rows else (0.0, 0.0)
        regime_accuracy_pct = 0.35 * reg_acc_raw + 0.65 * reg_acc_ewma

        # Regime-transition uncertainty relaxation: only relax in uncertain regime maps.
        try:
            regime_primary_prob = float((features or {}).get('regime_primary_prob', 1.0))
            if not np.isfinite(regime_primary_prob):
                regime_primary_prob = 1.0
        except Exception:
            regime_primary_prob = 1.0
        transition_relax = max(0.0, 0.60 - regime_primary_prob)
        acc_relax = np.clip(transition_relax * 20.0, 0.0, 4.0)
        edge_relax = np.clip(transition_relax * 0.20, 0.0, 0.04)
        eff_min_acc = max(float(min_accuracy_pct) - float(acc_relax), 48.0)
        eff_min_edge = max(float(min_avg_edge_pct) - float(edge_relax), 0.02)

        if accuracy_pct < eff_min_acc:
            reason = f'low_global_accuracy<{float(eff_min_acc):.1f}%'
            passed = False
        elif avg_edge_pct < eff_min_edge:
            reason = f'low_global_edge<{float(eff_min_edge):.2f}%'
            passed = False
        elif side_samples >= int(min_side_samples) and side_accuracy_pct < max(eff_min_acc - 2.0, 0.0):
            reason = 'low_side_accuracy'
            passed = False
        elif regime_samples >= int(min_regime_samples) and regime_accuracy_pct < max(eff_min_acc - 3.0, 0.0):
        ...
        return {
            ...
            'accuracy_pct_raw': round(float(accuracy_raw), 2),
            'accuracy_pct_ewma': round(float(accuracy_ewma), 2),
            'avg_edge_pct_raw': round(float(avg_edge_raw), 3),
            'avg_edge_pct_ewma': round(float(avg_edge_ewma), 3),
            'effective_min_accuracy_pct': round(float(eff_min_acc), 2),
            'effective_min_avg_edge_pct': round(float(eff_min_edge), 3),
            'regime_primary_prob': round(float(regime_primary_prob), 4),
            'side': side,
...
class TradePlanGenerator:
    ...
    def generate(omega_out, spot, lot_size=65, capital=500000):
        ...
        # Explicit round-trip transaction cost model for NSE options.
        bid = getattr(omega_out, 'bid', None)
        ask = getattr(omega_out, 'ask', None)
        try:
        ...
        if bid is not None and ask is not None and ask > bid > 0:
            ...
            spread = mp * 0.004  # 40 bps fallback spread when quote book unavailable
        stt = mp * 0.000625
        brokerage_per_unit = 20.0 / max(float(lot_size), 1.0)
        slippage = mp * 0.001
        round_trip_cost = spread / 2.0 + stt + brokerage_per_unit + slippage
        net_edge = abs(fv - mp) - round_trip_cost
        edge_covers_costs = bool(net_edge > 0.0)

        original_signal = signal
        if (('BUY' in str(signal).upper()) or ('SELL' in str(signal).upper())) and not edge_covers_costs:
            signal = 'HOLD'
        ...
        return {
            ...
            'edge_covers_costs': edge_covers_costs,
        }
...
class OMEGAModel:
    ...
    def __init__(self, nirv_model=None, data_dir='omega_data'):
        ...
        # --- Auto-learning / calibration controls ---
        self.auto_reconcile_delay_sec = 300.0   # 5 minutes
        self.auto_reconcile_max_age_sec = 6 * 3600.0
        self._quote_cache = {}  # {contract_key: {"ts": float, "price": float}}
        self._last_reconcile_ts = 0.0

        # Final-output conformal (calibrated on realized OMEGA fair-value residuals)
        self._omega_final_q_global = None
        self._omega_final_q_by_regime = {}
        self._omega_final_min_samples = 30
        self._direction_conf_calibrator = (
            ConfidenceCalibrator() if ConfidenceCalibrator is not None else None
        )
        self._direction_conf_last_fit_n = 0

        # Cold-start bootstrap from local historical artifacts
        self._bootstrap_attempted = False
        self._maybe_bootstrap_ml_from_local_history()
        self._rebuild_final_conformal_from_tracker()
        self._refresh_direction_calibrator(force=True)

    @staticmethod
    def _contract_key(option_type, strike, T):
    ...
    @staticmethod
    def _prediction_ts_epoch(pred_record):
        try:
            t = float(pred_record.get("ts_epoch"))
            if np.isfinite(t):
                return t
        except Exception:
            pass
        try:
            s = str(pred_record.get("ts", "") or "")
            if s:
                return float(datetime.datetime.fromisoformat(s).timestamp())
        except Exception:
            pass
        return None

    def _prediction_base_fv(self, pred_record):
        base = pred_record.get("base_fair_value", None)
        if base is None:
            base = (pred_record.get("pred") or {}).get("base_fair_value", None)
        try:
            v = float(base)
    ...
    def _rebuild_final_conformal_from_tracker(self):
        abs_res = []
        by_regime = defaultdict(list)
        for p in self.tracker.predictions:
            out = p.get("outcome") or {}
            pred = p.get("pred") or {}
            actual_price = out.get("actual_price", None)
            fair_value = pred.get("fair_value", None)
            try:
                ap = float(actual_price)
                fv = float(fair_value)
                if not (np.isfinite(ap) and np.isfinite(fv) and fv > 0):
                    continue
                r = abs((ap - fv) / fv)
                abs_res.append(r)
                regime = self.tracker._regime_from_features(p.get("features") or {})
                by_regime[regime].append(r)
            except Exception:
                continue

        if len(abs_res) >= self._omega_final_min_samples:
            self._omega_final_q_global = float(np.quantile(np.asarray(abs_res, dtype=float), 0.90))
        else:
            self._omega_final_q_global = None

        self._omega_final_q_by_regime = {}
        for regime, vals in by_regime.items():
            if len(vals) >= max(8, self._omega_final_min_samples // 3):
                self._omega_final_q_by_regime[regime] = float(np.quantile(np.asarray(vals, dtype=float), 0.90))

    def _omega_final_q(self, features):
        q = self._omega_final_q_global
        if q is None:
            return None
        regime = self.tracker._regime_from_features(features or {})
        q_reg = self._omega_final_q_by_regime.get(regime)
        if q_reg is not None:
            q = 0.6 * float(q) + 0.4 * float(q_reg)
        # Mild widening for near-expiry options where residuals are noisier
        T = max(float((features or {}).get("time_to_expiry", 0.05)), 1e-6)
        short_mult = 1.0 + 0.15 * np.clip((7.0 / 365.0) / T, 0.0, 2.0)
        return float(np.clip(q * short_mult, 0.0, 0.60))

    @staticmethod
    def _estimate_round_trip_cost(market_price, bid=None, ask=None, lot_size=65, volume_oi_ratio=None):
        mp = max(float(market_price), 0.01)
        try:
        ...
        brokerage_per_unit = 20.0 / max(float(lot_size), 1.0)
        try:
            voi = float(volume_oi_ratio) if volume_oi_ratio is not None else 0.1
        except Exception:
            voi = 0.1
        voi = float(np.clip(voi, 1e-4, 5.0))
        slippage_mult = 1.0 + 1.0 / max(np.sqrt(voi * 100.0), 1.0)
        slippage = mp * 0.001 * slippage_mult
        return float(spread / 2.0 + stt + brokerage_per_unit + slippage)

    def _refresh_direction_calibrator(self, force: bool = False) -> bool:
        """
        Refit confidence calibrator from realized directional outcomes.
        """
        if self._direction_conf_calibrator is None:
            return False

        samples = []
        for p in reversed(self.tracker.predictions[-1800:]):
            pred = p.get('pred') or {}
            out = p.get('outcome') or {}
            ...
            if 'actual_return' not in out:
                continue
            try:
                conf = float(pred.get('confidence_level'))
            except Exception:
                continue
            if not np.isfinite(conf):
                continue
            signed = PredictionTracker._signed_return_pct(sig, out.get('actual_return', 0.0))
            hit = 1 if signed > 0.0 else 0
            samples.append((float(np.clip(conf, 0.0, 99.0)), int(hit)))
        n = len(samples)
        if n < 60:
            return False
        if (not force) and n < (self._direction_conf_last_fit_n + 25):
            return False
        try:
            conf_arr = [s[0] for s in samples]
            hit_arr = [s[1] for s in samples]
            self._direction_conf_calibrator.fit(conf_arr, hit_arr)
            self._direction_conf_last_fit_n = n
            return True
        except Exception:
            return False

    def _calibrate_confidence_from_tracker(self, raw_confidence: float, signal: str) -> float:
        """
        Post-hoc confidence calibration using realized directional outcomes.

        Keeps the existing confidence engine but pulls long-run calibration error
        down as outcome history grows.
        """
        sig = str(signal or '').upper()
        if sig not in ('BUY', 'STRONG BUY', 'SELL', 'STRONG SELL'):
            return float(np.clip(raw_confidence, 0.0, 99.0))

        self._refresh_direction_calibrator(force=False)

        rows = []
        for p in reversed(self.tracker.predictions[-1200:]):
            pred = p.get('pred') or {}
            out = p.get('outcome') or {}
            psig = str(pred.get('signal', '')).upper()
            if psig not in ('BUY', 'STRONG BUY', 'SELL', 'STRONG SELL'):
                continue
            if 'actual_return' not in out:
                continue
            try:
                c = float(pred.get('confidence_level'))
            except Exception:
                continue
            if not np.isfinite(c):
                continue
            signed = PredictionTracker._signed_return_pct(psig, out.get('actual_return', 0.0))
            rows.append((float(np.clip(c, 0.0, 99.0)), 1.0 if signed > 0.0 else 0.0))
            if len(rows) >= 800:
                break

        if len(rows) < 80:
            if self._direction_conf_calibrator is not None:
                try:
                    return float(np.clip(self._direction_conf_calibrator.transform(raw_confidence), 1.0, 99.0))
                except Exception:
                    pass
            return float(np.clip(raw_confidence, 0.0, 99.0))

        conf_arr = np.asarray([r[0] for r in rows], dtype=float)
        hit_arr = np.asarray([r[1] for r in rows], dtype=float)
        bins = np.array([0, 20, 30, 40, 50, 60, 70, 80, 90, 100], dtype=float)
        b = int(np.clip(np.digitize([float(raw_confidence)], bins, right=False)[0] - 1, 0, len(bins) - 2))
        lo, hi = bins[b], bins[b + 1]
        mask = (conf_arr >= lo) & (conf_arr < hi)
        if np.sum(mask) < 12:
            nearest = np.argsort(np.abs(conf_arr - float(raw_confidence)))[:max(12, min(80, len(conf_arr)))]
            empirical_hit = float(np.mean(hit_arr[nearest]))
        else:
            empirical_hit = float(np.mean(hit_arr[mask]))

        calibrated = 0.60 * float(raw_confidence) + 0.40 * (100.0 * empirical_hit)
        if self._direction_conf_calibrator is not None:
            try:
                calibrated_iso = float(self._direction_conf_calibrator.transform(raw_confidence))
                calibrated = 0.55 * calibrated + 0.45 * calibrated_iso
            except Exception:
                pass
        return float(np.clip(calibrated, 1.0, 99.0))

    def _auto_reconcile_pending(self, now_ts):
        if now_ts - self._last_reconcile_ts < 1.0:
            return 0
        self._last_reconcile_ts = float(now_ts)

        to_close = []
        for p in reversed(self.tracker.predictions[-1500:]):
            if p.get("outcome"):
                continue
            pt = self._prediction_ts_epoch(p)
            if pt is None:
                continue
            age = float(now_ts) - float(pt)
            if age < self.auto_reconcile_delay_sec:
                continue
            if age > self.auto_reconcile_max_age_sec:
                continue
            ckey = p.get("contract_key") or (p.get("pred") or {}).get("contract_key")
            if not ckey:
                continue
            q = self._quote_cache.get(ckey)
            if not isinstance(q, dict):
                continue
            q_ts = float(q.get("ts", 0.0))
            q_px = float(q.get("price", np.nan))
            if not np.isfinite(q_px):
                continue
            if q_ts < pt + self.auto_reconcile_delay_sec:
                continue
            pred_mp = (p.get("pred") or {}).get("market_price", np.nan)
            try:
                pred_mp = float(pred_mp)
            except Exception:
                pred_mp = np.nan
            actual_ret = 0.0
            if np.isfinite(pred_mp) and pred_mp > 0:
                actual_ret = (q_px - pred_mp) / pred_mp
            to_close.append((p.get("id"), actual_ret, q_px))

        for pid, ar, ap in to_close:
            try:
                self.learn_from_outcome(pid, actual_return=ar, actual_price=ap)
            except Exception:
                continue
        return len(to_close)

    def _maybe_bootstrap_ml_from_local_history(self, min_rows=80, max_rows=600):
        if self._bootstrap_attempted:
            return
        self._bootstrap_attempted = True
        if not self.ml or bool(getattr(self.ml, "is_trained", False)):
            return

        try:
            import pandas as pd
            from historical_learning import HistoricalLearningConfig, _build_omega_features
        except Exception:
            return

        candidates = [
            os.path.join(self.data_dir, "historical", "processed", "features.parquet"),
            os.path.join("omega_data", "historical", "processed", "features.parquet"),
        ]
        ...
        try:
            ...
            return
        if df is None or df.empty or "residual_label" not in df.columns:
            return

        valid = df[np.isfinite(df["residual_label"].to_numpy(dtype=float))].copy()
        ...
        if len(valid) > int(max_rows):
            valid = valid.tail(int(max_rows))

        cfg = HistoricalLearningConfig(output_root=self.data_dir, model_path=os.path.join(self.data_dir, "pricing_model.joblib"))
        n_added = 0
        for row in valid.itertuples(index=False):
            row_s = pd.Series(row._asdict())
            try:
                feats = _build_omega_features(row_s, cfg)
                residual = float(row_s.get("residual_label", 0.0))
                arr = FeatureFactory.to_array(feats)
                self.ml.training_X.append(arr.tolist())
                self.ml.training_y.append(residual)
                if not hasattr(self.ml, "training_w"):
                    self.ml.training_w = []
                self.ml.training_w.append(1.0)
                n_added += 1
            except Exception:
                continue

        if n_added >= max(MLPricingCorrector.MIN_SAMPLES_BAYESIAN, min_rows):
            try:
                self.ml._train()
            except Exception:
                pass
    ...
    def price_option(self, spot, strike, T, r, q, option_type, market_price,
        ...
        """
        now_ts = kwargs.get("timestamp", None)
        try:
            now_ts = float(now_ts) if now_ts is not None else float(datetime.datetime.now().timestamp())
        except Exception:
            now_ts = float(datetime.datetime.now().timestamp())
        contract_key = self._contract_key(option_type=option_type, strike=strike, T=T)
        self._quote_cache[contract_key] = {"ts": now_ts, "price": float(market_price)}
        self._auto_reconcile_pending(now_ts)
        ...
        mdata = {
            ...
            'timestamp': kwargs.get('timestamp', now_ts),
        ...
        qx = {}
        try:
            qx = dict((nirv_result.greeks or {}).get('quant_extras') or {})
        except Exception:
            qx = {}
        regime_blend = qx.get('regime_blend', {}) if isinstance(qx, dict) else {}
        if isinstance(regime_blend, dict):
            primary_prob = regime_blend.get('primary_prob', regime_blend.get('primary_prob_pct', 100.0))
            try:
                primary_prob = float(primary_prob)
                if primary_prob > 1.0:
                    primary_prob = primary_prob / 100.0
            except Exception:
                primary_prob = 1.0
        else:
            primary_prob = 1.0
        features['regime_primary_prob'] = float(np.clip(primary_prob, 0.0, 1.0))
        features['regime_transition_uncertain'] = 1.0 if features['regime_primary_prob'] < 0.60 else 0.0
        ...
        if behavioral_context:
            ...
            if behav_impact != 0.0:
                max_prob = max((a.get('probability', 0.0) for a in behav_analysis.values()), default=0.0)
                if max_prob > 0.40:
                    behav_adj = nirv_result.fair_value * behav_impact * 0.03

        # ── COMBINE: OMEGA Fair Value ─────────────────────────────────
        omega_fv = nirv_result.fair_value * (1.0 + ml_corr * self.ml_weight)
        omega_fv += sent_adj + behav_adj
        omega_fv = max(omega_fv, 0.01)
        omega_fv_lo = max(nirv_result.fair_value * (1.0 + ml_corr_lo * self.ml_weight) + sent_adj + behav_adj, 0.01)
        omega_fv_hi = max(nirv_result.fair_value * (1.0 + ml_corr_hi * self.ml_weight) + sent_adj + behav_adj, 0.01)
        omega_final_q = None
        if getattr(get_features(), "USE_CONFORMAL_INTERVALS", False):
            omega_final_q = self._omega_final_q(features)
            if omega_final_q is not None and np.isfinite(omega_final_q) and omega_final_q > 0:
                lo_final = max(omega_fv * (1.0 - omega_final_q), 0.01)
                hi_final = max(omega_fv * (1.0 + omega_final_q), lo_final)
                omega_fv_lo = min(omega_fv_lo, lo_final)
                omega_fv_hi = max(omega_fv_hi, hi_final)

        omega_misp = (omega_fv - market_price) / max(market_price, 0.01) * 100
        round_trip_cost = self._estimate_round_trip_cost(
            market_price=market_price,
            bid=mdata.get('bid'),
            ask=mdata.get('ask'),
            lot_size=self.lot_size,
            volume_oi_ratio=mdata.get('volume_oi_ratio'),
        )
        net_edge = abs(omega_fv - market_price) - round_trip_cost
        edge_covers_costs = bool(net_edge > 0.0)
        edge_after_cost_pct = float(100.0 * net_edge / max(market_price, 1e-6))
        net_edge_norm = float(np.clip(net_edge / max(round_trip_cost, 1e-6), 0.0, 3.0) / 3.0)
        interval_width_pct = float(
            100.0 * max(omega_fv_hi - omega_fv_lo, 0.0) / max(market_price, 0.01)
        )
        ...
        if getattr(get_features(), "USE_CONFORMAL_INTERVALS", False):
            ...
            voi = float(np.clip(mdata.get('volume_oi_ratio', 0.0), 0.0, 5.0))
            liq_score = 0.60 * np.clip(voi / 0.06, 0.0, 1.0) + 0.40 * np.clip((0.24 - spread_ratio) / 0.24, 0.0, 1.0)
            liquidity_ok = bool(liq_score >= 0.35)
            if not liquidity_ok:
                actionability_reasons.append("LOW_LIQUIDITY_FOR_CONFORMAL_GATE")
            base_edge_pct = float(kwargs.get("min_conformal_edge_pct", 0.40))
            adaptive_edge_pct = base_edge_pct + min(0.80, spread_ratio * 100.0 * 0.15)
            if voi < 0.03:
                adaptive_edge_pct += 0.35
            elif voi < 0.06:
                adaptive_edge_pct += 0.15
            min_edge_abs = max(
                market_price * adaptive_edge_pct / 100.0,
                0.35 * spread,
                0.03,
            )
        ...
        # Cost-aware actionability: suppress directional calls that do not clear round-trip costs.
        if omega_signal != 'HOLD' and not edge_covers_costs:
            omega_signal = 'HOLD'
            actionability_reasons.append("EDGE_BELOW_TRANSACTION_COST")

        omega_conf = nirv_result.confidence_level
        if ml_conf > 0:
            omega_conf = 0.70 * nirv_result.confidence_level + 0.30 * (ml_conf * 100)
        omega_conf = self._calibrate_confidence_from_tracker(omega_conf, omega_signal)

        sig_up = str(omega_signal).upper()
        directional = sig_up in ('BUY', 'STRONG BUY', 'SELL', 'STRONG SELL')
        phys_p = float(np.clip(nirv_result.physical_profit_prob / 100.0, 0.01, 0.99))
        if 'BUY' in sig_up:
            direction_prob = phys_p
        elif 'SELL' in sig_up:
            direction_prob = 1.0 - phys_p
        else:
            direction_prob = 0.50
        direction_conf_calibrated = float(np.clip(direction_prob, 0.0, 1.0))
        if self._direction_conf_calibrator is not None:
            try:
                direction_conf_calibrated = float(np.clip(
                    self._direction_conf_calibrator.transform(direction_prob * 100.0) / 100.0,
                    0.0,
                    1.0,
                ))
            except Exception:
                pass
        if directional:
            signed_edge_pct = edge_after_cost_pct if 'BUY' in sig_up else -edge_after_cost_pct
            expected_return_1d_pct = float(np.clip(0.35 * signed_edge_pct, -20.0, 20.0))
        else:
        ...
        conviction_score_10 = self._research_conviction_score_10(
            ...
            net_edge_norm=net_edge_norm,
            uncertainty_width_pct=interval_width_pct,
        )
        high_conviction_min_score = int(np.clip(kwargs.get("high_conviction_min_score", 8), 0, 10))
        high_conviction_actionable = bool(conviction_score_10 >= high_conviction_min_score)

        # Research-only mode: keep output focused to high-conviction picks.
        if (
        ...
        if oos_gate.get('required'):
            if oos_gate.get('passed'):
                oos_bucket = "passed"
            else:
                oos_bucket = f"blocked:{str(oos_gate.get('reason', 'unknown'))}"
        else:
            oos_bucket = "not_required"

        # Recompute directional heads on the final post-gate signal.
        sig_up = str(omega_signal).upper()
        directional = sig_up in ('BUY', 'STRONG BUY', 'SELL', 'STRONG SELL')
        if 'BUY' in sig_up:
            direction_prob = phys_p
        ...
        self.tracker.record(pred_id, features, {
            'signal': omega_signal,
            'fair_value': omega_fv,
            'base_fair_value': nirv_result.fair_value,
            'market_price': market_price,
            'mispricing': omega_misp,
            'confidence_level': omega_conf,
            'conviction_score_10': int(conviction_score_10),
            'conformal_actionable': bool(conformal_actionable),
            'round_trip_cost': float(round_trip_cost),
            'net_edge': float(net_edge),
            'edge_after_cost_pct': float(edge_after_cost_pct),
            'edge_covers_costs': bool(edge_covers_costs),
            'direction_prob': float(direction_prob),
            'direction_conf_calibrated': float(direction_conf_calibrated),
            'expected_return_1d_pct': float(expected_return_1d_pct),
            'oos_bucket': str(oos_bucket),
        ...
        return OMEGAOutput(
            ...
            direction_conf_calibrated=round(float(direction_conf_calibrated), 4),
            expected_pnl=nirv_result.expected_pnl,
            physical_expected_pnl=nirv_result.physical_expected_pnl,
            expected_return_1d_pct=round(float(expected_return_1d_pct), 3),
            # Base
            ...
            final_conformal_q_pct=round(float(omega_final_q * 100.0), 2) if omega_final_q is not None else None,
            conformal_actionable=bool(conformal_actionable),
            conviction_score_10=int(conviction_score_10),
            ...
            behavioral_adjustment=round(behav_adj, 2),
            ...
            ask=mdata.get('ask'),
            volume_oi_ratio=mdata.get('volume_oi_ratio'),
            round_trip_cost=round(float(round_trip_cost), 3),
            net_edge=round(float(net_edge), 3),
            edge_after_cost_pct=round(float(edge_after_cost_pct), 3),
            edge_covers_costs=bool(edge_covers_costs),
            oos_bucket=str(oos_bucket),
            interval_low=round(float(omega_fv_lo), 2),
            interval_high=round(float(omega_fv_hi), 2),
            # Learning
    ...
    def _research_conviction_score_10(
        ...
        conformal_actionable=True,
        net_edge_norm=0.0,
        uncertainty_width_pct=None,
    ):
        """
        Research scorer on a full 0-10 scale.

        Uses a bounded composite score with interaction terms so aligned
        edge + probability gets rewarded while low-ML-confidence outliers
        get penalized.
        """
        sig = str(signal or "").upper()
        ...
        pop = float(np.clip(abs(float(physical_pop) - 50.0) / 50.0, 0.0, 1.0))
        conf = float(np.clip(float(confidence_level) / 100.0, 0.0, 1.0))
        eff = float(np.clip(float(efficiency_score) / 100.0, 0.0, 1.0))
        mlc = float(np.clip(float(ml_confidence), 0.0, 1.0))
        edge = float(np.clip(float(net_edge_norm), 0.0, 1.0))
        ...
        composite += 0.05 * misp * pop
        composite -= 0.04 * (1.0 - mlc) * misp
        composite += 0.08 * edge
        if misp > 0.60 and pop > 0.35 and conf > 0.70:
            composite += 0.08
        if uncertainty_width_pct is not None:
            try:
                unc = float(max(float(uncertainty_width_pct), 0.0))
            except Exception:
                unc = 0.0
            # Penalize very wide fair-value intervals.
            composite -= float(np.clip((unc - 12.0) / 40.0, 0.0, 0.12))
        if "STRONG" in sig:
            composite += 0.04
        ...
        return int(np.clip(round(composite * 11.5), 0, 10))
        ...
        for p in reversed(self.tracker.predictions):
            if p['id'] == prediction_id and p.get('features'):
                if self.ml:
                    ...
                    base_fv = self._prediction_base_fv(p) or 0.0
    ...
    def get_status(self):
        ...
        return {
            ...
            'min_samples_bayesian': MLPricingCorrector.MIN_SAMPLES_BAYESIAN,
            'min_samples_gbm': MLPricingCorrector.MIN_SAMPLES_GBM,
            ...
            'ml_ensemble_enabled': bool(getattr(get_features(), "USE_ML_ENSEMBLE", False)),
            'ml_ensemble_models': (
                sorted(list((self.ml._ensemble_models or {}).keys()))
                if self.ml and hasattr(self.ml, "_ensemble_models")
                else []
            ),
            'auto_reconcile_delay_sec': float(self.auto_reconcile_delay_sec),
            'auto_reconcile_max_age_sec': float(self.auto_reconcile_max_age_sec),
            'quote_cache_size': len(self._quote_cache),
            'final_conformal_global_q_pct': (
                round(float(self._omega_final_q_global * 100.0), 3)
                if self._omega_final_q_global is not None else None
            ),
            'final_conformal_regimes': {
                str(k): round(float(v * 100.0), 3)
                for k, v in (self._omega_final_q_by_regime or {}).items()
            },
        }
    ...
    def scan_chain(self, spot, strikes, T, r, q,
        ...
        if getattr(get_features(), "USE_RESEARCH_HIGH_CONVICTION", False):
            # Research mode: keep output sparse and above configured conviction floor.
            high_conviction_min_score = int(np.clip(kwargs.get("high_conviction_min_score", 8), 0, 10))
            results = [
                x for x in results
                if getattr(x[2], "conviction_score_10", 0) >= high_conviction_min_score
            ]
            results.sort(
                key=lambda x: (
                    getattr(x[2], "conviction_score_10", 0),
                    float(getattr(x[2], "net_edge", 0.0)),
        ...
        if bool(getattr(get_features(), "USE_ENHANCED_RANKING", False)):
            def _adjusted_edge_key(item):
                ...
                net_edge = max(float(getattr(out, "net_edge", 0.0)), 0.0)
                ...
                edge = misp * pop * conf * max(eff, 0.1) * actionable * (1.0 + min(net_edge / 10.0, 1.0))
                return (
                    float(getattr(out, "conviction_score_10", 0)),
                    edge,
                    net_edge,
        ...
        # Preserve baseline ordering for regression safety when enhanced ranking
        # and research high-conviction modes are both disabled.
