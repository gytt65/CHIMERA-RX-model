...
# ── Import the TVR model lazily (avoids Streamlit side-effects) ─────
TVRModel = None


def _ensure_tvr_model_loaded():
    global TVRModel
...
# Keep TVR listed as supported; availability is resolved lazily.
...
class NirvBacktester:
    ...
    def __init__(self, initial_capital=500000, max_positions=3,
                 ...
                 omega_min_conviction=8, omega_require_cost_cover=True,
                 require_cost_cover=True):
        """
        Parameters
        ----------
        ...
        self.omega_min_conviction = int(np.clip(omega_min_conviction, 0, 10))
        self.omega_require_cost_cover = bool(omega_require_cost_cover)
        self.require_cost_cover = bool(require_cost_cover)
            ...
            if len(positions) < self.max_positions and i >= 5 and regime_ok:
                ...
                for sig in signals:
                    if len(positions) >= self.max_positions:
                        break
                    if (not bool(sig.get('omega_signal_filtered', False))) and abs(sig['mispricing_pct']) < self.signal_threshold:
                        continue
    ...
    def _generate_signals(self, snap, model=None):
        ...
        for K in target_strikes:
            for opt_type in ['CE', 'PE']:
                ...
                if model is not None and model != 'tvr':
                    ...
                    try:
                        if hasattr(model, 'price_option'):
                            ...
                            if self.model_type == 'omega':
                                sig_up = str(signal).upper()
                                if 'BUY' not in sig_up:
                                    continue
                                conviction = int(getattr(result, 'conviction_score_10', 0))
                                if conviction < int(self.omega_min_conviction):
                                    continue
                                if self.omega_require_cost_cover and not bool(getattr(result, 'edge_covers_costs', True)):
                                    continue
                                expected_net_edge_pct = float(getattr(result, 'edge_after_cost_pct', np.nan))
                                if not np.isfinite(expected_net_edge_pct):
                                    expected_net_edge_pct = self._expected_net_edge_pct(ask_price, fair_value)
                                if self.require_cost_cover and expected_net_edge_pct <= 0.0:
                                    continue
                                mispricing_pct = float(getattr(result, 'mispricing_pct',
                                                               (fair_value - market_price) / max(market_price, 1e-8) * 100.0))
                                signals.append({
                                    'strike': K,
                                    ...
                                    'mispricing_pct': round(float(mispricing_pct), 2),
                                    'omega_signal_filtered': True,
                                    'conviction_score_10': conviction,
                                    'expected_net_edge_pct': round(float(expected_net_edge_pct), 2),
                                    'edge_covers_costs': bool(getattr(result, 'edge_covers_costs', True)),
                                })
                                continue
                    except Exception:
                        # Fallback to BSM
                ...
                elif model == 'tvr' and _ensure_tvr_model_loaded() is not None:
                    # TVR PDE pricing
                    try:
                        pricer = TVRModel(
                ...
                if signal == 'BUY':
                    ...
                    signals.append({
                        ...
                        'expected_net_edge_pct': round(float(expected_net_edge_pct), 2),
                        'edge_covers_costs': bool(expected_net_edge_pct > 0.0),
                    })
    ...
    def _expected_net_edge_pct(self, entry_price, fair_value):
        """
        Expected net edge estimate with conservative round-trip cost model.
        """
        ep = max(float(entry_price), 0.01)
        fv = max(float(fair_value), 0.01)
        # Approximate realized exit around modeled fair value with mild slippage.
        exit_ref = max(0.5, fv * 0.98)
        cost_per_unit = float(self._compute_costs(ep, exit_ref, 1) / max(self.lot_size, 1))
        net_edge_per_unit = float(fv - ep - cost_per_unit)
        return float(100.0 * net_edge_per_unit / ep)
