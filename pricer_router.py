...
class TieredPricerRouter:
    ...
    def route_price(
        ...
        *,
        ...
        now = time.time()
        key = self._cache_key(spot, strike, T, r, q, option_type, surface_iv)
        cached = self._cache.get(key)
        if cached is not None and (now - float(cached.get("_ts", 0.0))) <= self._cache_ttl_s:
            out = dict(cached)
            out["cache_hit"] = True
            return out
