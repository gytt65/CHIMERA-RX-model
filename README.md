...
9. [Detailed Module Documentation](#-detailed-module-documentation)
    ...
    - [CHIMERA-RX Shadow Engine (`CHIMERA_RX_MODEL_DETAILED.md`)](#chimera-rx-shadow-engine-chimera_rx_model_detailedmd)
...
### CHIMERA-RX Shadow Engine (`CHIMERA_RX_MODEL_DETAILED.md`)
CHIMERA-RX is an additive advisory layer and does not mutate OMEGA/NIRV/TVR outputs.
- **Runtime Role**: Shadow diagnostics only (`chimera_rx_tab.py`), with abstention/readiness gating.
- **Core Method**: `CHIMERARXEngine.run(CHIMERARXInput)` from `chimera_rx_engine.py`.
- **Outputs**: Equilibrium/deformed/tail prices, compression action (`abstain`), PTD alert, quality score (`Q_t`), readiness checks.
- **Canonical Documentation**: See `CHIMERA_RX_MODEL_DETAILED.md` for architecture, formulas, test status, known issues, and current deployment guidance.
...
| `CHIMERA_RX_MODEL_DETAILED.md` | CHIMERA reference | Shadow architecture, math, tests, limits, readiness |
